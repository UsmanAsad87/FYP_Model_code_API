...
def transfer_learning():
    ...
    new_model.compile(
        loss= 'categorical_crossentropy',#ArcLoss(),#
              optimizer='adam',
              metrics=['accuracy'])

    cp1= ModelCheckpoint(filepath='Saved_model', monitor='loss',     save_best_only=True, verbose=1, mode='min')
    callbacks_list = [cp1]

    # print('here2')
    hist=new_model.fit(train_ds,validation_data=val_ds,epochs=25, callbacks=callbacks_list)
    print('HISTORY::')
    print(hist.history)
